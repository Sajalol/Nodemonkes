...
class Command(BaseCommand):
    ...
    def fetch_new_listings(self, collection_symbol, kind="list", limit=10):
        url = f"https://api-mainnet.magiceden.dev/v2/ord/btc/activities?collectionSymbol={collection_symbol}&kind={kind}&limit={limit}"
        headers = {"accept": "application/json"}
        response = requests.get(url, headers=headers)
        print("Response status code:", response.status_code)
        print("Response content:", response.text)
...
'''
    def handle(self, *args, **kwargs):
        ...
        new_listings = self.fetch_new_listings('nodemonkes')
        if new_listings:
            print("New Listings Data:", new_listings)
            self.stdout.write(self.style.SUCCESS('Successfully fetched new listings'))
    '''
